...
class MainScreen(Tk):
    ...
    def actualizar_botones(self, estados_led):
        botones = [self.Cuarto1, self.Cuarto2, self.Cuarto3, self.Cuarto4]
        for i, estado in enumerate(estados_led):
            if estado == '1':
                botones[i].config(text=f"Apagar Cuarto{i+1}", fg="#c0392b")
            else:
                botones[i].config(text=f"Encender Cuarto{i+1}", fg="#229954")
